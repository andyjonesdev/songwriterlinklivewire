                ...
                <table class="w-full text-sm">
                    <thead>
                        <tr class="border-b border-sidebar-border/70 dark:border-sidebar-border text-left text-gray-500">
                            ...
                            <th class="pb-2 pr-4">Matched URL</th>
                    ...
                    <tbody>
                        @foreach ($lyrics as $lyric)
                            <tr class="border-b border-sidebar-border/70 dark:border-sidebar-border">
                                ...
                                <td class="py-2 pr-4 text-xs max-w-xs truncate">
                                    @if ($lyric->plagiarism_match_url)
                                        <a href="{{ $lyric->plagiarism_match_url }}" target="_blank" class="text-blue-600 hover:underline">
                                            {{ $lyric->plagiarism_match_url }}
                                        </a>
                                    @else
                                        —
                                    @endif
                                </td>
                                <td class="py-2 pr-4 text-gray-500 text-xs max-w-xs">{{ $lyric->plagiarism_flag_reason ?? '—' }}</td>
                                <td class="py-2">
